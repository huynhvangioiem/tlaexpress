...
export default function MenuQLUser() {
...
// Handle customTab
function CustomTab({ children, to }) {
  let resolved = useResolvedPath(to);
  let match = useMatch({ path: resolved.pathname, end: true });
  return (
    <Link to={to}>
      <div className={match ? clsx("row", style.tabItem, style.active) : clsx("row", style.tabItem)}>
        <div className={clsx("col-2 col-m-2 col-s-12", style.icon)}><FontAwesomeIcon icon="fa-solid fa-circle-plus" /></div>
...
function ShowTab(props) {
  ...
  var result = tabs.map((tab, index) => <CustomTab key={index} to={ChangeToSlug(tab)}>{tab}</CustomTab>)
  return (
    <>{result}</>
  )
}